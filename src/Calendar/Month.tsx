...
import {
  customGetDay,
  monthWidth,
} from "@/components/Calendar/consts";
import { endOfMonth, getDaysInMonth } from "date-fns";
import { pipe } from "fp-ts/function";
import * as NEA from "fp-ts/NonEmptyArray";
import { memo } from "react";
import { SmartDay } from "@/components/Calendar/SmartDay";
import { getDateKey } from "@/components/Calendar/date-key";
import {
  useCalendarContext,
  useComponent,
  useI18n,
} from "@/components/Calendar/Context";
import { MonthName } from "@/components/Calendar/Components/MonthName";
import { WeekDays } from "@/components/Calendar/Components/WeekDays";
...
export const Month = memo(({ monthDate }: Props) => {
  ...
  const days = pipe(
    NEA.range(
      0,
      numberOfDays + firstWeekday + (6 - lastWeekday) - 1,
    ),
    NEA.map((i) => {
      if (i < firstWeekday) {
  ...
  return (
    <View style={styles.container}>
      {hideYear ? null : (
        <Year year={monthDate.getFullYear()} />
      )}
